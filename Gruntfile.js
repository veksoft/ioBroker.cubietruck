...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Veksoft';
    var email     = grunt.option('email')  || 'veksoft@gmail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new cubietruck name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/cubietruck.png')) {
            fs.renameSync(__dirname + '/admin/cubietruck.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/cubietruck.html')) {
            fs.renameSync(__dirname + '/widgets/cubietruck.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/cubietruck/js/cubietruck.js')) {
            fs.renameSync(__dirname + '/widgets/cubietruck/js/cubietruck.js', __dirname + '/widgets/cubietruck/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/cubietruck')) {
            fs.renameSync(__dirname + '/widgets/cubietruck',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Cubietruck/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Cubietruck'
                        },
                        {
                            match: /Veksoft/g,
                            replacement: author
